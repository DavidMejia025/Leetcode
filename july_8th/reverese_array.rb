...
# You may assume all the sacters consist of printable ascii sacters.
#
# @param {Character[]} s
# @return {Void} Do not return anything, modify s in-place instead.

# brute force # this shit works but hell slow that almost broke the pc
# Good learning:
...
# brute force O(n) time and O(2n) space
def reverse_string(s)
  length     = s.length
  iterations = (length)
...
# easy to understand letcode alternative:
 def reverse_string(s)
    be=0
    e=s.length-1
    while be < e
...
# Recursion variant
# Bad idea, in this case the task is never accomplished by reducing the size of the input and requires to use almost all the elements. Suitable for loops (iteration)
def reverse_string2(s)
    s = s.shift

    return [s] if s == []

    return reverse_string(s).push(s)
end

#After reviewing solutions in let code it turns out that there is a way to do this recursevely but with another array lets say space complexity > o(n). At least is good
